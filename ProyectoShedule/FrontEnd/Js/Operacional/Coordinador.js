function loadTable() {
    $.ajax({
        url: "http://localhost:9000/api/v1/shedule/operacional/coordinador/",
        method: "GET",
        headers: {
        ...
        items.forEach(function (Elementos, posicion, array) {
            variable += `<tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-warning" onclick="findById(${Elementos.id})" data-bs-toggle="modal" data-bs-target="#modalCoordinador"><i class='bx bx-search'></i></button>
...
function findById(id) {
    $.ajax({
        url: "http://localhost:9000/api/v1/shedule/operacional/coordinador/" + id,
        method: "GET",
        headers: {
            "Content-Type": "application/json"
        }
