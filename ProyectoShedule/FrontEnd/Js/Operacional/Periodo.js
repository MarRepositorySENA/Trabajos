function loadTable() {
    $.ajax({
        url: "http://localhost:9000/api/v1/shedule/operacional/periodo/",
        method: "GET",
        headers: {
        ...
        items.forEach(function (Elementos, posicion, array) {
            variable += `<tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-warning" onclick="findById(${Elementos.id})" data-bs-toggle="modal" data-bs-target="#modalPeriodo"><i class='bx bx-search'></i></button>
...
function findById(id) {
    $.ajax({
        url: "http://localhost:9000/api/v1/shedule/operacional/periodo/" + id,
        method: "GET",
        headers: {
            "Content-Type": "application/json"
        }
